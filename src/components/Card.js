...
const Card = ({ movie }) => {
   ...
   const deleteStorage = () => {
      ...
      let newData = storedData.filter((id) => id != movie.id);  // permet de filtrer et de ne stocker dans newData que ce qui est différent de id. id étant le film à supprimer
   ...
   return (
      <div className="card">
         ...
         {movie.genre_ids ? (
            ...
            <div className="btn" onClick={() => deleteStorage()}>Supprimer du coup de coeur</div>
         )}
      </div>
   );
};
